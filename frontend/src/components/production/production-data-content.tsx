...
const getStatusBadge = (status: ProductionEntryStatus) => {
  const styles: Record<ProductionEntryStatus, string> = {
    [ProductionEntryStatus.PENDING]:
      "bg-yellow-500 text-yellow-50 font-semibold",
    [ProductionEntryStatus.APPROVED]:
      "bg-green-600 text-green-50 font-semibold",
  ...
  return (
    <span className={`... ${styles[status]} px-2 rounded-lg py-1`}>
      {labels[status]}
    </span>
  );
};
...
export const ProductionDataContent: React.FC<ProductionDataContentProps> = ({
  user,
    ...
    import("xlsx")
      .then((XLSX) => {
        ...
        const exportData = entries.map((entry) => {
          ...
          return {
            ...
            ...(userRole === "coordinator"
              ? { Partner: partner?.organization || partner?.name || "N/A" }
              : {}),
        ...
        const startDate = filters.start_date
          ? formatShortDate(filters.start_date, false)
          : "All";
        const endDate = filters.end_date
          ? formatShortDate(filters.end_date, true)
          : "Data";
  ...
  const columns = React.useMemo<Column<ProductionEntry>[]>(
    () => [
      {
        ...
        render: (row: ProductionEntry) => (
          <span className="font-mono">
            {formatShortDate(row.measurement_date)}
          </span>
        ),
      },
      ...(userRole === "coordinator"
        ? [
            {
              ...
              render: (row: ProductionEntry) => {
                ...
                return (
                  <span>{partner?.organization || partner?.name || "..."}</span>
                );
              },
        ...
        : []),
      {
        ...
        render: (row: ProductionEntry) => (
          <span className="font-mono">
            {formatVolumeWithCommas(row.gross_volume)}
          </span>
        ),
      },
      {
        ...
        render: (row: ProductionEntry) => {
          ...
          return (
            <span className="font-mono">
              {formatVolumeWithCommas(netVolume)}
            </span>
          );
        },
      },
    ...
    [userRole, partners, handleReject]
